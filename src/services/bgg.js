...
const REGEX_REGOLE = /regol|regole|istruzion|rulebook|\brules?\b|\bmanual/i;
const REGEX_ITALIANO = /\b(ita|italiano|italian)\b|regol|regole|istruzion/i;

/**
 * Cerca tra i file di un gioco il regolamento migliore.
 * Priorità: regole in italiano > regole in qualsiasi lingua.
 * Ritorna { titolo, url, italiano } oppure null se non trova regole
 * (in quel caso il chiamante apre la pagina file generica).
 *
 * NB: usa l'endpoint privato api.geekdo.com/api/files (non la XML API pubblica).
 */
export async function trovaRegolamento(bggId) {
  const { data } = await axios.get("https://api.geekdo.com/api/files", {
    params: {
      objectid: bggId,
      objecttype: "thing",
      pageid: 1,
  ...
  for (const f of files) {
    const testo = `${f.title ?? ""} ${f.filename ?? ""}`;
    if (!REGEX_REGOLE.test(testo)) continue; // solo file di regole

    const italiano = f.languageid === LANG_ITALIANO || REGEX_ITALIANO.test(testo);
    const voti = Number(f.numpositive) || 0;
    // tier 2 = regole ITA, tier 1 = regole altra lingua; a parità più votato
    const tier = italiano ? 2 : 1;

    if (
      !migliore ||
      tier > migliore.tier ||
      (tier === migliore.tier && voti > migliore.voti)
    ) {
      migliore = { tier, voti, italiano, titolo: f.title || f.filename, href: f.href };
    }
  }
  ...
  return {
    titolo: migliore.titolo,
    italiano: migliore.italiano,
    url: `https://boardgamegeek.com${migliore.href}`
  };
}
